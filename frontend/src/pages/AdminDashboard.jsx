...
function AdminDashboard({ setIsSignIn, setRoleId }) {
  ...
  return (
    <div className="min-h-screen p-6" style={{ backgroundColor: "var(--bg-primary)", color: "var(--text-primary)" }}>
      ...
      {message && (
        <div
          className={`rounded-md p-4 mb-6 text-sm ${messageType === "error" ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}
          style={{ border: "1px solid var(--border)", backgroundColor: messageType === "error" ? "rgba(248, 113, 113, 0.12)" : "rgba(52, 211, 153, 0.16)", color: messageType === "error" ? "var(--severity-critical)" : "var(--status-resolved)" }}
        >
          {message}
        </div>
      )}
      ...
      {isModalOpen && selectedUser && (
        <div className="fixed inset-0 flex items-center justify-center" style={{ backgroundColor: "rgba(15, 23, 42, 0.6)" }}>
          <div className="p-6 rounded-lg shadow-md w-96" style={{ backgroundColor: "var(--card-bg)", color: "var(--text-primary)", border: "1px solid var(--border)" }}>
